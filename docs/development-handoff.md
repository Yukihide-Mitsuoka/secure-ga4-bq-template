...
| Active work | PR #317 and Issue #316 are complete. Issue #318 proposes making `intermediate` optional while preserving the v2 default to prevent unintended Dataset deletion. ADR-0012 is awaiting repository-owner review; implementation must remain a separate PR after approval. No cloud resource is required. Acceptance S remains condition-gated. | [Issue #318](https://github.com/Yukihide-Mitsuoka/secure-ga4-bq-template/issues/318), [ADR-0012](adr/0012-make-intermediate-layer-optional.md), [System overview](requirements/README.md) |
...
4. Apply the asset to a second engagement only when an owner, target scope, customer
