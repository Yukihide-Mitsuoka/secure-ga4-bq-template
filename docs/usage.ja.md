...
*   @your-username
```
個人リポジトリにチーム記法を残すと、CODEOWNERS が**黙って無効化**されます。互換ラッパーは
アカウント種別を判定しないため、ガバナンス反映前にこのファイルを修正してください。
...
既存の有効順序を維持します。このリポジトリは、常時結果を報告することを確認済みの
`iac-scan` contextを追加する `terraform-gcp` profileを継承し、leaf repository policyでは
共有チェックを重複定義しません。
...
`scripts/setup-github.sh` はポリシーを持たない互換ラッパーです。`DRY_RUN` が空でなければ
`plan` へ委譲し、通常実行では同じ対象名を2回要求して `apply` へ委譲します。`gh` を直接呼ばず、
reconcilerの終了コードをそのまま返します。従来の引数なし形式とスクリプト内のオンボーディング案内は
廃止したため、このガイドをチェックリストとして使用してください。
...
### ソロ開発 × 過去に適用した従来の初期設定 ＝ 自分のPRをマージできない
従来の固定bootstrapはレビュー1件をadminにも強制しました。現在の `scripts/setup-github.sh` は
階層ポリシーへ委譲し、その固定値を持ちません。階層ポリシーの既定値は、
