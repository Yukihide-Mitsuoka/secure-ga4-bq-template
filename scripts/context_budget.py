...
REQUIRED_READS = {
    ...
    "requirements": {
        ".ai/mission.md",
        ".ai/documentation.md",
        "docs/foundation/templates/requirements.md",
    },
...
def validate_adr_index(root: Path) -> list[str]:
    ...
    entries = [
        match.groupdict() for match in ADR_ROW_PATTERN.finditer(index.read_text(encoding="utf-8"))
    ]
    actual_targets = {path.name for path in directory.glob("[0-9][0-9][0-9][0-9]-*.md")}
    ...
    for entry in entries:
        ...
        if not target.startswith(f"{entry['number']}-"):
            errors.append(f"foundation ADR index: label {entry['number']} does not match {target}")
        ...
        for field, key in (("status", "status"), ("updated", "date")):
            ...
            if not indexed_value:
                errors.append(f"foundation ADR index: {target}: {field.title()} is empty")
...
def validate_guide_index(root: Path) -> list[str]:
    ...
    entries = [
        match.groupdict() for match in GUIDE_ROW_PATTERN.finditer(index.read_text(encoding="utf-8"))
    ]
    actual_targets = {path.name for path in directory.glob("*.md") if path.name != "README.md"}
    indexed_targets = [entry["target"] for entry in entries]
    errors = [
...
def handoff_warnings(root: Path, *, current_date: date) -> list[str]:
    ...
    try:
        ...
        warnings.append("development handoff has a missing or invalid ISO updated date")
        return warnings

    age = (current_date - updated_date).days
    if age < 0:
        warnings.append(f"development handoff updated date is {abs(age)} days in the future")
    elif age > HANDOFF_STALE_DAYS:
        warnings.append(
            f"development handoff may be stale: updated {age} days ago > {HANDOFF_STALE_DAYS}"
        )
    return warnings
...
def active_baseline_files(root: Path) -> tuple[list[str], tuple[str, ...]]:
    """Resolve the explicit agent profile without recursive discovery."""
    errors: list[str] = []
    files = list(BASELINE_FILES)
    guardrail_entry = root / ".ai/guardrails.md"
    if guardrail_entry.is_file() and CANONICAL_GUARDRAILS in guardrail_entry.read_text(
        encoding="utf-8"
    ):
    ...
    if set(profile) != {"schema_version", "authority_policy", "inputs"}:
        errors.append("agent profile must contain only schema_version, authority_policy, inputs")
    ...
    if not isinstance(inputs, list) or len(inputs) < 2:
        errors.append("agent profile.inputs must contain foundation and project inputs")
        return errors, tuple(files)
...
def validate_conditional_authorities(
    ...
    for contract in contracts:
        ...
        if target_error:
            errors.append(
                f"conditional authority {contract.name}: {contract.target}: {target_error}"
            )
        else:
        ...
        for reference, markers in contract.references:
            ...
            if reference_error:
                errors.append(
                    f"conditional authority {contract.name}: {reference}: {reference_error}"
                )
                continue
            normalized = " ".join((root / reference).read_text(encoding="utf-8").split())
            errors.extend(
                f"conditional authority {contract.name}: "
...
def measure_skill_route(
    ...
    duplicate_reads = sorted(value for value, count in Counter(reads).items() if count > 1)
    if duplicate_reads:
    ...
    if baseline_duplicates:
        errors.append(f"{skill_file}: redundantly rereads baseline: {baseline_duplicates}")
...
def audit(
    ...
    *,
    ...
    actual_skills = {path.name.removesuffix(".skill.md"): path for path in skill_files}
...
def main() -> int:
    ...
    for name, counts in report["conditional_routes"].items():
        print(f"context budget: conditional={name} {counts.bytes} bytes, {counts.words} words")
