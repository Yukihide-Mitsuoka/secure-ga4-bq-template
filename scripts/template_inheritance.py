...
def _bootstrap_export(parent_root, parent_repository, source_commit):
    export_path, export = _bootstrap_export_path(parent_repository, parent_root, source_commit)
...
def _bootstrap_desired(child_root, repository, parent_repository, source_commit, export):
    ...
    _validate_agent_input_order(inputs, parent_repository)
    _validate_agent_input_ownership(inputs, export["inherited_paths"], export["protected_paths"])
    ignore = {"docs/**"}
    ignore.update(f"{path}**" if path.endswith("/") else path for path in export["protected_paths"])
    if _owned_by("docs/foundation/", export["inherited_paths"]):
        ignore.update({"docs/**", ":!docs/foundation/", ":!docs/foundation/**"})
    return {
        "manifest": {
            "schema_version": 2,
...
def plan_bootstrap(root, parent_root, source_commit, repository):
    ...
    desired = _bootstrap_desired(child_root, repository, parent_repository, source_commit, export)
    return {
        "schema_version": SCHEMA_VERSION,
...
def _validate_bootstrap_manual_payloads(payloads, repository, parent_repository, source_commit):
    ...
    if repository not in overlay or "{{" in overlay:
        raise InheritanceError(
            "bootstrap project overlay must identify the child without placeholders"
        )
    ...
    if re.search(r"(?<!\$)\{\{", workflow) or any(
        ...
        raise InheritanceError(
            "bootstrap Template Sync workflow has invalid direct-parent settings"
        )
    ...
    if not archive.startswith(expected_frontmatter) or README_OWNER_MARKER.findall(archive) != [
        parent_repository
    ]:
...
def _finalization_review(child_root, parent_root, contract, source_commit):
    ...
    if source_commit != contract["parent"]["commit"]:
        for path in _changed_paths(parent_root, contract["parent"]["commit"], source_commit):
            owner = _path_owner(path, contract["ownership"])
...
def _finalization_report(contract, child_repository, branch, source_commit, review):
    has_unsupported_manual_port = any(
        item["reason"] != "workflow-security-boundary" for item in review["pending_manual_port"]
    )
    ...
    return {
        "schema_version": SCHEMA_VERSION,
        "status": (
            "blocked" if is_blocked else "ready_to_finalize" if has_work else "already_finalized"
        ),
...
def apply_finalization(
    ...
    *,
    ...
    payloads = [_manual_port_payload(parent_root, source_commit, path) for path in manual_paths]
...
def _fleet_repository(repository, child_root, parent_root):
    ...
    if candidate:
        ...
        for path in plan["changes"]["add"] + plan["changes"]["modify"]:
            ...
            if child_entry == target_entry:
                ...
                pending_manual_port.append({"path": path, "reason": _manual_transport_reason(path)})
            else:
        ...
        for path in plan["skipped"]["unowned"]:
            ...
            if child_entry is None and target_entry is None:
                continue
            ownership_review.append({"path": path, "reason": "ownership-decision-required"})
    else:
        ...
        for path in audited_paths:
            ...
            is_manual = _template_sync_excludes(path, excluded, exceptions)
            if parent_entry is None:
                deletion_review.append({"path": path, "reason": "deletion-review-required"})
            elif child_entry == parent_entry:
                destination = manually_ported if is_manual else synchronized
                destination.append(path)
            elif is_manual:
    ...
    return {
        ...
        "pending_manual_port": sorted(pending_manual_port, key=lambda item: item["path"]),
...
def _read_fleet_config(config_path):
    ...
    try:
        ...
        config = json.loads(resolved.read_text(encoding="utf-8"), object_pairs_hook=_unique_object)
    except (OSError, UnicodeError, json.JSONDecodeError) as error:
    ...
    if (
        ...
        raise InheritanceError(f"fleet config.schema_version must be {FLEET_SCHEMA_VERSION}")
    ...
    for index, entry in enumerate(repositories):
        ...
        parent_repository = _repository(entry["parent_repository"], f"{label}.parent_repository")
        directory = _fleet_directory(entry["directory"], f"{label}.directory")
        parent_directory = _fleet_directory(entry["parent_directory"], f"{label}.parent_directory")
        lifecycle = entry["lifecycle"]
        if type(lifecycle) is not str or lifecycle not in FLEET_LIFECYCLES:
            raise InheritanceError(f"{label}.lifecycle must be active, paused, or retired")
        reason = entry["reason"]
        if (
    ...
    for item in validated:
        parent_lifecycle = lifecycle_by_repository.get(item["parent_repository"].casefold())
...
def fleet_audit(config_path, workspace_root):
    ...
    for index, entry in enumerate(entries):
        ...
        if contract["parent"]["repository"].casefold() != entry["parent_repository"].casefold():
            raise InheritanceError(
    ...
    entry_by_repository = {entry["repository"].casefold(): entry for entry in entries}
...
def propagation_impact(
    ...
    changes = _propagation_changes(entries, workspace_root, parent_repository, changed_paths)
    status = max(
    ...
    return {
        ...
        "changes": changes,
        "summary": {
            impact: sum(item["impact"] == impact for item in changes) for impact in IMPACT_PRIORITY
        },
    }
        ...
        if args.command == "validate":
            ...
            if args.apply:
                ...
                if args.confirm_repository or args.confirm_source:
                    raise InheritanceError("confirmation arguments are accepted only with --apply")
                report = plan_finalization(args.root, args.parent_root, args.source_commit)
